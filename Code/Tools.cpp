...
BWAPI::Unit Tools::GetClosestUnitTo(BWAPI::Position p, const BWAPI::Unitset& units)
{
    ...
    for (auto& u : units)
    {
        if (!closestUnit || u->getDistance(p) < closestUnit->getDistance(p)) // Changed here 
        {
            closestUnit = u;
        }
    }
...
BWAPI::Unit Tools::GetUnitOfType(BWAPI::UnitType type, BWAPI::Unit badBuilder)
{
    // For each unit that we own
    for (auto& unit : BWAPI::Broodwar->self()->getUnits())
    {
        // if the unit is of the correct type, and it actually has been constructed, return it
        if (unit->getType() == type && unit->isCompleted() && unit != badBuilder)
        {
            return unit;
        }
    }
...
BWAPI::Unit Tools::GetClosestUnitOfType(BWAPI::UnitType type, BWAPI::Position position)
{
    // For each unit that we own
    BWAPI::Unit closestUnit = nullptr;
...
BWAPI::Unit Tools::GetClosestUnitOfTypeBeingBuilt(BWAPI::UnitType type, BWAPI::Position position)
{
    // For each unit that we own
    BWAPI::Unit closestUnit = nullptr;
    float distance = 100000;
...
BWAPI::Unit Tools::GetDepot()
{
    ...
    return GetUnitOfType(depot, nullptr);
}
...
bool Tools::BuildBuilding(BWAPI::UnitType type)
{
    ...
    // float distance = builder->getDistance(pos);
    ...
    BWAPI::Position buildPosPos(buildPos);

    BWAPI::Unit builder = Tools::GetClosestUnitOfType(builderType, buildPosPos);
    if (!builder) { return false; }


    // if (builder->getLastCommand().getTargetTilePosition() == buildPos) { return false; }

    // if (!BWAPI::Broodwar->canBuildHere(buildPos, type, builder)) { return false; }
